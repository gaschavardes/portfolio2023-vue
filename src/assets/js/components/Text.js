...
export default class Text extends Group {
	...
	enter() {
		...
		gsap.to(this.mesh.material.uniforms.uAppear, { value: 1, duration: store.isMobile ? 3 : 2, ease: 'power1.Out'})
	}
	leave() {
		gsap.to(this.mesh.material.uniforms.uAppear, { value: 0, duration: store.isMobile ? 2 : 1, ease: 'power1.Out',
			onComplete: () => {
